...
/**
 * This class implements a basic algorithm for performing search of program without using counterexamples
 * @author Pablo
 *
 */
...
//	HashMap<String,Boolean> cexRefined;  
	 ...
	 * @param templatePath
	 */
	public SimpleSearch(Spec mySpec, String outputPath, String templatePath, boolean showInfo, boolean printPDF, int scope){
	 ...
	 * @param templatePath
	 */
	public SimpleSearch(Spec mySpec, String outputPath, String templatePath, boolean showInfo, boolean printPDF, int scope, boolean alloySearch, int pathBound){
		this.syntProgram = "";
		this.mySpec = mySpec;
...
//		this.queueCexs = (LinkedList<LinkedList<LinkedList<String>>>[]) new LinkedList[instancesList.size()];
	...
	public void startSearch(){
		System.out.println("Using Simple Search without Use of Counterexamples for Synthesis...");
		...
		//}
		// The simple search is started
		boolean found = simpleSearch(0, scope);
		if (found){
	...
	public boolean simpleSearch(int insNumber, int scope){
		...
		if (insNumber == (this.numberIns - 1)){ //  Base Case
			...
			A4Solution currentSol =  this.getAlloySolution(currentIns);
			//this.queueSolvers[insNumber].addLast(this.getAlloySolution(currentIns));
			int j = 0;
			while (currentSol.satisfiable()){ 
				this.iterations++;
				try{
					currentSol.writeXML(outputPath+"temp"+j+".xml");
				}
				catch (Exception e){
					System.out.println(e);
				}
				...
				if (showInfo) 
					System.out.println("Instance "+ currentIns + ", Iteration Number:"+iterations);
			...
			while (solver.satisfiable()){  // !disjointCexFound.get(currentIns)??
				  ...
				  p++;
				  mapInsModels.put(currentIns, lts); // we try with this model recursively if
	...
	private boolean selectChecker(String currentIns){
		boolean result = false;
		if (this.alloySearch)
			result = alloyBoundedModelCheck(currentIns, new LinkedList<LinkedList<String>>(), this.pathBound, this.scope);
		if (this.nuSMVSearch)
			result = nuSMVModelCheck(currentIns, new LinkedList<LinkedList<String>>());
		if (this.electrumSearch || this.nuXMVSearch)
			result = this.electrumBoundedModelCheck(currentIns, new LinkedList<LinkedList<String>>(), this.pathBound, this.scope);
		if (!this.alloySearch && !this.electrumSearch && !this.nuXMVSearch && !this.nuSMVSearch)
			result = modelCheck(currentIns, new LinkedList<LinkedList<String>>());
		return result;
	}
