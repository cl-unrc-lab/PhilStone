...
* Author: Pablo.
*/
w1  : prim_boolean;  /* w is the lock for the writer 1 */ 
r1  : prim_boolean;  /* w is the lock for the reader 1 */ 
r2  : prim_boolean;  /* w is the lock for the reader 2 */ 
r3  : prim_boolean;  /* w is the lock for the reader 3 */ 
r4  : prim_boolean;  /* w is the lock for the reader 4 */ 

process writer1{
	enum st = {Writing, Waiting}; /* the writer can be waiting or reading */
	owns : w1;  /* this flag is only modified by this processor */
	init: this.st = Waiting  &&  !global.r1 && !global.r2 && !global.r3 && !global.r4 && !global.w1 ;
    
	/* Writer's action for adquiring the lock */
	action startWriting(){
		frame: w1, st;
		pre: !global.w1;
		post: global.w1 && (this.st = Writing);
	}
	...
	action stopWriting(){
		frame: w1, st;
		pre: global.w1;
		post: !global.w1 && (this.st = Waiting);
	}
