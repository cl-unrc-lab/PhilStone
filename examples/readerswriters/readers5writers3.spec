...
r5  : prim_boolean;  /* r5 is the lock for the reader 5 */ 

process writer1{
	enum st = {Writing, Waiting}; /* the writer can be waiting or reading */
	init: this.st = Waiting  &&  !global.r1 && !global.r2 && !global.r3 && !global.r4 && !global.r5 && av(global.w);
...
process reader5{
	enum st = {Reading, Waiting}; /* the reader is reading or waiting */
	owns: r5; /* this flag is only modified by this process */
	init: (this.st = Waiting) &&  !global.r1 && !global.r2 && !global.r3 && !global.r4 && !global.r5 && av(global.w);

	action startReading(){
		frame: st, r5;
		pre: this.st = Waiting;
		post: this.st = Reading && global.r5;
	}

	action stopReading(){
		frame:  st, r5;
		pre: this.st = Reading;
		post: this.st = Waiting && !global.r5;
	}
...
property :AG[!(pr1.st=Reading&&pw1.st=Writing)]&&AG[!(pr1.st=Reading&&pw2.st=Writing)]&&AG[!(pr1.st=Reading&&pw3.st=Writing)]&&AG[!(pr2.st=Reading&&pw1.st=Writing)]&&AG[!(pr2.st=Reading&&pw2.st=Writing)]&&AG[!(pr2.st=Reading&&pw3.st=Writing)]&&AG[!(pr3.st=Reading&&pw1.st=Writing)]&&AG[!(pr3.st=Reading&&pw2.st=Writing)]&&AG[!(pr3.st=Reading&&pw3.st=Writing)]&&AG[!(pr4.st=Reading&&pw1.st=Writing)]&&AG[!(pr4.st=Reading&&pw2.st=Writing)]&&AG[!(pr4.st=Reading&&pw3.st=Writing)]&&AG[!(pr5.st=Reading&&pw1.st=Writing)]&&AG[!(pr5.st=Reading&&pw2.st=Writing)]&&AG[!(pr5.st=Reading&&pw3.st=Writing)]&&AG[!(pw1.st=Writing&&pw2.st=Writing)]&&AG[!(pw1.st=Writing&&pw3.st=Writing)]&&AG[!(pw2.st=Writing&&pw3.st=Writing)] && EF[(pr1.st = Reading)||(pr2.st = Reading)||(pr3.st = Reading)||(pr4.st = Reading)||(pr5.st = Reading)||(pw1.st = Writing)||(pw2.st = Writing)||(pw3.st = Writing)]&& AG[!(pw1.st = Writing) || EF[pw1.st = Waiting]]&& AG[!(pw2.st = Writing) || EF[pw2.st = Waiting]]&& AG[!(pw3.st = Writing) || EF[pw3.st = Waiting]]&& AG[!(pr1.st = Reading) || EF[pr1.st = Waiting]]&& AG[!(pr2.st = Reading) || EF[pr2.st = Waiting]]&& AG[!(pr3.st = Reading) || EF[pr3.st = Waiting]]&& AG[!(pr4.st = Reading) || EF[pr4.st = Waiting]]&& AG[!(pr5.st = Reading) || EF[pr5.st = Waiting]];