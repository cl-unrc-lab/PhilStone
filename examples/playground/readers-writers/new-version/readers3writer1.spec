...
read1, read2:lock;  /* this is a global atribute, used for signaling that the reader 1 or reader 2 are reading */

process writer{
	enum st = {Writing, Waiting};
	init: this.st = Waiting  && av(global.r) && av(global.read1) && av(global.read2);
...
property: AG[!(r1.st = Reading && w1.st = Writing) && !(r2.st = Reading && w1.st = Writing)]; 
