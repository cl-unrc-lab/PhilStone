...
grant_keeps_high_until_up = "&&".join([f"""G[!global.r{i} || !p{i}.g{i} || [p{i}.g{i} W !global.r{i}]]""" for i in range(1,n+1)])
grant_keeps_low_until_up  = "&&".join([f"""G[global.r{i} || p{i}.g{i} || [!p{i}.g{i} W global.r{i}]]""" for i in range(1,n+1)])
grant_request_coincide = "&&".join(["G[F[(global.r{i} && p{i}.g{i}) || (!global.r{i} && !p{i}.g{i})]]" for i in range(1,n+1)])

file.write("property: "+ liveness + "\n &&" + safety +"\n &&"+grant_keeps_high_until_up +"\n &&"+ grant_keeps_low_until_up +"\n &&"
           + grant_request_coincide +";\n")
